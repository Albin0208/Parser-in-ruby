...
class TestInterpreterVar < Test::Unit::TestCase
  ...
  def test_evaluate_var_declaration
    ...
    assert_instance_of(Runtime::Values::NumberVal, @env.identifiers['x'])
    ...
    assert_instance_of(Runtime::Values::NullVal, @env.identifiers['empty'])
    ...
    result = @interpreter.evaluate(ast, @env)
    assert_equal(5, result.value)
    assert_instance_of(Runtime::Values::NumberVal, @env.identifiers['t'])
    ...
    assert_instance_of(Runtime::Values::NumberVal, @env.identifiers['y'])
    ...
    assert_instance_of(Runtime::Values::BooleanVal, @env.identifiers['b'])
    ...
    assert_instance_of(Runtime::Values::StringVal, @env.identifiers['str'])
    assert_equal('Hello', @env.identifiers['str'].value)
  end

  def test_evaluate_retrieval_of_var
    @env.declare_var('x', Runtime::Values::NumberVal.new(10, :int), 'int', 1, false) # Declare the var in the env
  ...
  def test_evaluate_var_assignment_expr
    @env.declare_var('x', Runtime::Values::NumberVal.new(10, :int), 'int', 1, false)
    input = 'x = 5'
    ast = @parser.produce_ast(input)
    result = @interpreter.evaluate(ast, @env)
    assert_equal(5, result.value)
  ...
  def test_evaluate_int_and_float_assignment_conversion
    ...
    @env.declare_var('y', Runtime::Values::NumberVal.new(10.3, :float), 'float', 1, false)
    input = 'y = 5'
    ast = @parser.produce_ast(input)
    result = @interpreter.evaluate(ast, @env)
    assert_equal(5, result.value)
  ...
  def test_evaluate_invalid_var_assignment_expr
    @env.declare_var('x', Runtime::Values::NumberVal.new(10, :int), 'int', 1, false)
    assert_raise(RuntimeError) { @env.declare_var('x', Runtime::Values::NumberVal.new(10, :int), false, 'int') }

    # Test reassign of const value
    @env.declare_var('c', Runtime::Values::NumberVal.new(10, :int), 'int', 1, true)
    input = 'c = 5'
    ast = @parser.produce_ast(input)
